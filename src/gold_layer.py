...
def criar_tabelas_para_consumidores(
    ...
    df = spark.read.format("parquet").load(
        f"abfss://{container_silver}@{storage_account_name}.dfs.core.windows.net/*.parquet"
    )

    df.cache()

    df_dim_brewery_table = df.select(
        "name", "phone", "is_phone_missing", "fixed_phone", "website_url"
    ).distinct()
...
def gravar_tabelas_no_banco(
    df: DataFrame, table_name: str, connection_properties: str
):
    ...
    return print(f"A tabela {table_name} foi salva com sucesso!")
